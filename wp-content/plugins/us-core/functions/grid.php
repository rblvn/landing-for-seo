				...
				if ( strpos( $param, $prefix ) === 0 ) {
					...
					foreach ( $param_values as $item_value ) {
						if (
							...
							$result[ $source_prefix . '|' . $param_name ][] = $item_value;
						}
					}
				}
			}
...
if ( ! function_exists( 'us_apply_grid_filters' ) ) {
	...
	function us_apply_grid_filters( $post_id, &$query_args, $grid_filter_params = NULL ) {
		...
		if ( ! $post_grid_filter_atts = get_post_meta( $post_id, '_us_grid_filter_atts', TRUE ) ) {
			if ( ! $post_grid_filter_atts = us_save_post_grid_filter_atts( $post_id ) ) {
				$post_grid_filter_atts = get_post_meta( us_get_page_area_id( 'content' ), '_us_grid_filter_atts', TRUE );
			}
			if ( ! $post_grid_filter_atts ) {
		...
		foreach ( us_arr_path( $query_args, 'tax_query', array() ) as $term ) {
			...
			$terms = is_array( $term['terms'] ) ? $term['terms'] : array( $term['terms'] );
		...
		foreach ( $current_tax_queries as $item_name => $item_values ) {
			$query_args['tax_query'][] = array(
				'taxonomy' => $item_name,
				'field' => ( is_numeric( $item_values ) OR ( isset( $item_values[0] ) AND is_numeric( $item_values[ 0 ] ) ) )
					? 'id'
					: 'slug',
				'terms' => $item_values,
				'operator' => 'IN',
...
if ( ! function_exists( 'us_define_content_and_apply_grid_filters' ) ) {
	 ...
	 * @return void
	 */
	function us_define_content_and_apply_grid_filters( &$query_vars, $tax_query ) {
		global $_us_content_id;
		$grid_filter_found = FALSE;
		if ( ! $_us_content_id ) {
			$_us_content_id = us_get_page_area_id( 'content' );
			// Search filter in Page Blocks
			if ( $post = get_post( $_us_content_id ) AND strpos( $post->post_content, '[us_grid_filter' ) === FALSE ) {
...
if ( ! function_exists( 'us_inject_grid_filter_to_archive_page' ) ) {
	...
	function us_inject_grid_filter_to_archive_page( $query ) {
		...
		if (
			...
			us_define_content_and_apply_grid_filters( $query->query_vars, $query->tax_query );
		}
	}
