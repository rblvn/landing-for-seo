...
$.each(this.breakpoints,function(breakpointWidth,breakpointArgs){if(breakpointArgs!==undefined&&breakpointArgs.items!==undefined){this.carouselOptions.responsive[breakpointWidth]=breakpointArgs;this.carouselOptions.responsive[breakpointWidth].items=parseInt(breakpointArgs.items)}}.bind(this));this.$list.on('initialized.owl.carousel',function(e){$('[data-toggle-height]',e.currentTarget).each(function(_,item){var usToggle=$(item).data('usToggleMoreContent');if(usToggle instanceof $us.ToggleMoreContent){usToggle.initHeightCheck()}})}).on('mousedown.owl.core',function(){var $target=$(this);if($('[data-toggle-height]',$target).length&&!jQuery.isMobile){var owlCarousel=$target.data('owl.carousel');owlCarousel.$stage.off('mousedown.owl.core')}});this.$list.owlCarousel(this.carouselOptions)}.bind(this))}
...
if(uiType==='range'){var value=$('input[type="hidden"]:first',$item).val();if(value){hasValue=!!value;title+=': '+value}}
